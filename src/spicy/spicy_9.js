...
export const repeatDemo = () => {
    repeat((word) => console.log(word), 10, ...["Hello World!"])
};
...
export const filter = (arr, test) => {
    return arr.reduce((array, item) => {
        (test(item)) ? array.pass.push(item) : array.fail.push(item);
        return array;
    }, {pass: [], fail: []})
};
...
export const allEvensAreOdd = (arr) => {
    return everyEven(arr, item => (item % 2));
};
...
export const anEvenIsOdd = (arr) => {
    return someEven(arr, item => (item % 2));
};
...
export const hasExactly = (arr, test, n) => {
    return filter(arr, item).pass.length == n;
};
